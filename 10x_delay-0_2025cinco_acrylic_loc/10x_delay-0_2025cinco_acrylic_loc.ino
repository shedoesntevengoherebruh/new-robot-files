...
int delayMove = 0; // the number of MILLISECONDS for delays between MOVEMENTS/directions.... related to wiggle.
bool moveFlag = false;
...
const int center_x_1 = 14650; // changed from 13315 after moving hole in acrylic over//X distance from starting point to X center of first well (upper left) in encoder steps
...
void gen_and_collect(){
  // center();
  long z_encoder = 2000;//originally 1750
  move_z_steppers(300,1);
  move_z_steppers(z_encoder,1);
  delay(delayMove);
  for(int movement_iterator=0;movement_iterator<10000;movement_iterator++){
