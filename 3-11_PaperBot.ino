...
int z_top = 1000; //in steps, from the bottom position. equivalent to 5".
int Z_HOME = 4000; //comfortable height where needles are retracted and won't brush the plate, in terms of encoder steps.
float inch_rot = 1; // the number of lateral inches moved per rotation is 1. yes, inches.
...
int delaymotor = 500; // the number of microseconds used for delays between steps.
bool moveFlag = false;
...
const int center_x_1 = 3550; //X distance from starting point to X center of first well (upper left) in encoder steps
const int center_y_1=7100; // Y distance from starting point to Y center of first well in encoder steps.
...
void move_x_stepper(long number_of_encoder_counts, int dir){
  ...
  if (dir == 0){
    ...
    while (abs(global_target_encoderPositionx - encoderPositionx)>5){
...
void move_y_stepper(int number_of_encoder_counts, int dir){
  ...
  if (dir == 0){
    ...
    while (abs(global_target_encoderPositiony - encoderPositiony)>5){
...
void move_z_steppers(int encoder_counts, int dir){
  ...
  while ((abs(global_target_encoderPosition1- encoderPosition1)>10)|(abs(global_target_encoderPosition1- encoderPosition2)>10)){
    counter_z1 = 0;
    counter_z2 = 0;
    if (abs(global_target_encoderPosition1 - encoderPosition1)>10){
      fudge_steps_1 = (int)((((float)(encoderPosition1-global_target_encoderPosition1))/enc_rot)*act_rot);
    }
    else{
      fudge_steps_1 = 0;
    }
    if (abs(global_target_encoderPosition1 - encoderPosition2)>10){
...
void calibrateMotors() {
  while (digitalRead(Calibration_BUTTON)==LOW){
    Serial.println(digitalRead(Calibration_BUTTON));
  }
...
void gen_and_collect(){
  // center();
  int buttonState = digitalRead(Calibration_BUTTON); //be reading the calibration button state
  
  if (moveFlag == HIGH & finishedFlag == false) {
